Link: <https://contest.yandex.ru/contest/29075/enter/>
...
def main():
    ...
    first = sorted(list((i, value) for i, value in zip(first_row[1:], range(first_row[0]))), key=lambda x: (x[0], x[1]))
    second = sorted(list((i, value) for i, value in zip(second_row[1:], range(second_row[0]))), key=lambda x: (x[0], x[1]))
    third = sorted(list((i, value) for i, value in zip(third_row[1:], range(third_row[0]))), key=lambda x: (-x[0], x[1]))

    flag = False
    best = (len(first) + 1, len(second) + 1, len(third) + 1)
    for f in first:
        t = 0
        for s in second:
            while t < len(third) - 1 and f[0] + s[0] + third[t][0] > number_to_get:
                t += 1
            if f[0] + s[0] + third[t][0] == number_to_get and (not flag or (f[1], s[1], third[t][1]) < best):
                best = (f[1], s[1], third[t][1])
                flag = True
