Link: <https://contest.yandex.ru/contest/28970/enter/>
...
штата голосуют в соответствии с результатами голосования внутри штата, то есть на заключительной стадии выборов в
...
def main():
    ...
    if votes_left:
        sorted_parties = [x[0] for x in sorted(parties.items(), key=lambda x: -x[1]['decimal_part'])]
        i = 0
        while votes_left > 0:
