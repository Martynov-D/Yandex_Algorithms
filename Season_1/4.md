Link: <https://contest.yandex.ru/contest/27665>
...
def getWordStress(word: str):
    '''
    Возвращает множество индексов букв под ударением
    '''
    stresses = set()
...
def main():
    with open('input.txt', 'r') as f:
        ...
        for _ in range(number):
            ...
            dictionary[lowercase] = dictionary.get(lowercase, set())
        ...
        if len(wordStress) == 1:
            ...
            if lowercase in dictionary:
                if wordStress.pop() not in dictionary[lowercase]:
                    # Ошибка, если слово есть в словаре, у него всего одно ударение, но оно не совпадает
                    errors += 1
        else:
            # Ошибка, если несколько ударений
            errors += 1
    print(errors)
```
